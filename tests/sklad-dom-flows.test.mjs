...
test('Sklad date fields use the rounded custom date picker instead of the native popup', () => {
  ...
  assertIncludes(skladHtml, 'label for="issueDateI"', 'issue date label must be explicitly associated');
  assertIncludes(skladHtml, 'inputmode="none" autocomplete="off" pattern="\\d{4}-\\d{2}-\\d{2}" class="inp" id="issueDateI" name="issue_date"', 'issue date field must not use native date popup');
