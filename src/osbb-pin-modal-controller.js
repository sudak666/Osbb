import { appendPinDigit, deletePinDigit, isPinComplete } from './pin-entry.js';
...
export function createOsbbPinModalController(deps) {
    ...
    const modal = () => element('pin-modal');
    const updateDots = () => {
        for (let i = 0; i < 4; i++) element('pin-d' + i)?.classList.toggle('is-entered', i < buffer.length);
    };
    const focusDialog = () => {
        const dialog = modal()?.querySelector('[role="dialog"]');
        const firstButton = dialog?.querySelector(FOCUSABLE_SELECTOR);
    ...
    function show(nextCallback, options = {}) {
        buffer = '';
        busy = false;
    ...
    function handleKeydown(event) {
        ...
        const dialog = modal()?.querySelector('[role="dialog"]');
        if (!dialog) return;
        const focusables = [...dialog.querySelectorAll(FOCUSABLE_SELECTOR)].filter(control => control.offsetParent !== null || control === doc.activeElement);
