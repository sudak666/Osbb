...
Це не замінює ручну перевірку в браузері, але швидко ловить випадкове видалення важливих фрагментів коду та незавершені merge-conflict маркери (`<<<<<<<`, `=======`, `>>>>>>>`).

## Якщо GitHub показує merge conflict

1. Відкрийте файл із конфліктом і залиште тільки потрібний фінальний текст без службових рядків `<<<<<<<`, `=======`, `>>>>>>>`.
2. Для README зазвичай потрібно залишати обидва корисні блоки: порядок SQL, smoke-перевірки та наступні розділи про PWA/кешування.
3. Після ручного редагування запустіть `node scripts/smoke-check.mjs`; якщо маркери конфлікту залишились, команда покаже файл із проблемою.
4. Тільки після успішної перевірки робіть commit/merge.
