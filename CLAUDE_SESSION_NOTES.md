...
> **ОНОВЛЕННЯ (кінець липня 2026): секцію 2 нижче ("дві Supabase бази") скасовано.** Журнал і склад об'єднані в один Supabase-проєкт. Актуальний опис архітектури — в `README.md` і `CLAUDE.md`, не тут. Секцію 2 лишено нижче в оригінальному вигляді як історичний контекст (чому колись були саме два проєкти), а не як дійсну інструкцію — не діяти за нею.
...
- `supabase/` — **історичний архів** видаленого окремого проєкту журналу (див. `supabase/README.md`), не діюча схема.
- `sklad/supabase/` — актуальні SQL-міграції й Edge Functions **єдиного** проєкту (журнал + склад), пронумеровані `001_...` → `005_...`.
...
## 2. (ІСТОРИЧНО, вже неактуально) Дуже важливо: дві Supabase бази

У застосунку **раніше** використовувались дві різні Supabase бази / проєкти — з липня 2026 об'єднані в один (`vkwkyhjjjmcpmiakxohw`), окремий проєкт журналу видалено. Нижче — оригінальний текст нотатки, для історії:
...
Проєкт Складу (тепер — єдиний проєкт для обох модулів):

```text
vkwkyhjjjmcpmiakxohw
```

~~Не виконувати SQL для цін у основній базі журналу ОСББ.~~ Актуально: SQL для цін стосується тільки таблиці `inventory_items`, журнальних таблиць не зачіпає — але виконується в тому самому (єдиному) проєкті.
...
```text
sklad/supabase/004_add_price_tracking.sql
```

(перейменовано з `add_price_tracking.sql` під час нумерації SQL-файлів у липні 2026 — зміст файлу той самий)
...
1. ~~Не плутати Supabase базу Складу з основною базою ОСББ~~ — з липня 2026 неактуально, база одна.
2. Не прибирати `004_add_price_tracking.sql` — без нього ціни не збережуться.
