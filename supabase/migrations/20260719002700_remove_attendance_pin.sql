-- Mirror of: sklad/supabase/027_remove_attendance_pin.sql
-- Keep both migration trees identical below this header.

-- Табель зберігається без staff-PIN на явний запит власника застосунку.
-- RPC лишає перевірку місяця, дня, ролі та формату часу, але більше не
-- встановлює особу редактора.

drop function if exists public.save_attendance_day(text, integer, text, text, text, text, text, uuid, text);
...
revoke execute on function public.save_attendance_day(text, integer, text, text, text, text, text) from public;
grant execute on function public.save_attendance_day(text, integer, text, text, text, text, text) to anon, authenticated, service_role;
