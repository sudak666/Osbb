...
function readOsbbCombined() {
  return [
    ...
    'src/osbb-elevator-controller.js',
    'src/osbb-garbage.ts',
    'src/osbb-garbage-controller.js',
    'src/osbb-runtime-controller.js',
...
const checks = [
  ...
  ['src/osbb-attendance-controller.js', 'if (!isDispatcher() || !getPin()) {', 'attendance requests staff authentication only when saving'],
  ['src/osbb-attendance-controller.js', "showToast('Збереження скасовано: потрібен PIN керування');", 'attendance discards unsaved edits when PIN confirmation is cancelled'],
  ...
  ['styles.css', '.shell-desktop-actions { display: none; }', 'shell mobile navigation prioritizes primary modules'],
  ['shell', "doc.querySelectorAll<HTMLButtonElement>('[data-shell-lock]')", 'shell binds desktop and mobile lock actions'],
...
// secondary filters into one outlined disclosure.
{
  ...
  const required = [
    ...
    'class="pill items-filter-pill items-availability-toggle is-success md-state-layer"',
...
// visible/editable on the receipt that established the current item price.
{
  ...
  const required = [
    ...
    'async function add()',
    'function render()',
    "SUPPLIER_TAGS_STORAGE_KEY = 'sklad_supplier_tags_v1'",
...
// Графік змін працює як нативна M3-вкладка Журналу на спільному Supabase.
{
  ...
  const required = [
    ...
    '.shift-dot.is-half {',
    "db.from('work_shifts').select('*').eq('month_key', monthKey)",
...
// перемикається на вертикальні day cards без горизонтального свайпу.
{
  ...
  const required = [
    ...
    'adjacentDays.leading.map(renderAdjacentDay)',
    'adjacentDays.trailing.map(renderAdjacentDay)',
    'class="att-calendar-day is-adjacent-month" aria-disabled="true"',
    ...
    'днів завершено',
    'data-att-field="breakStart"',
    'data-att-field="breakEnd"',
    "data-att-absence-action=\"${hasAbsence ? 'clear' : 'add'}\"",
    "if (Boolean(next.breakStart) !== Boolean(next.breakEnd)) return;",
    "breakStart:'', breakEnd:''",
    "p_break_start:next.breakStart || ''",
    ...
    '.att-calendar-day.is-adjacent-month {',
