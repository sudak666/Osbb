...
## AI assistant, implemented (July 2026)

User explicitly asked to build the AI assistant specifically ("роби ші асистент тільки") — the one item from the teardown flagged as needing a scoping conversation first. Scope decisions made without further back-and-forth, documented here so they're visible rather than silently assumed:

- **One assistant per app, not one cross-app assistant.** `osbb/index.html` and `sklad/index.html` are separate iframes with separate concerns (journal vs. warehouse) — building a single UI that queries across both would need a cross-iframe messaging layer for no clear benefit at this scale. Each app got its own ambient query bar, scoped to its own data.
- **Read-only Q&A only, no action-taking.** The assistant answers questions ("хто чергує завтра?", "що закінчується?") — it does not create requests, log entries, or write anything. Letting an LLM interpret free text into a database write is a meaningfully bigger, riskier feature (confirmation flows, guarding against misread intent) that deserves its own separate pass, not something to bundle in silently.
- **UI pattern**: an ambient `.ai-bar` (orb icon + input + "Запитати" button + 3 example chips + answer panel) sits at the top of the primary content area in both apps — not a floating chat bubble, not a separate page/tab. Matches the "AI-first without visual noise" principle from the earlier design guide. Answer is rendered via `.textContent` only, never `.innerHTML` (LLM output is untrusted text, same rule as any other dynamic content).
- **Backend**: one new Edge Function, `sklad/supabase/functions/ai-assistant/index.ts`, parameterized by `{ question, scope: 'journal'|'sklad' }` — mirrors `notify-telegram`'s "one function, two contexts" pattern rather than deploying two near-identical functions. It reads the relevant tables with the service-role key (server-side, bypasses RLS — same trust level as the rest of this internal, PIN-gated tool) and passes them as raw context to Claude API (`claude-haiku-4-5-20251001`, chosen for cost/latency on short factual Q&A), with a system prompt that explicitly tells it not to invent facts not present in the data. Deploy/secret instructions added to `README.md`, matching the existing `fetch-item-prices`/`notify-telegram` documentation pattern exactly.
- **Not deployed by this session** — writing the Edge Function code doesn't deploy it, and deploying requires the user's own `ANTHROPIC_API_KEY` (an API key is not something an agent should generate or set on the user's behalf). Until deployed + the secret is set, the UI is fully visible and usable but any question shows a clear, specific error ("AI assistant is not configured...") instead of failing silently — verified with a real click in a headless browser against the actual (currently-undeployed) URL.
- Added a smoke-check guard (`AI assistant bar wired in both apps...`) checking the Edge Function file exists, both apps call it with the correct `scope`, and neither renders the answer via `innerHTML`.
