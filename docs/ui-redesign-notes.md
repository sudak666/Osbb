...
- **Next**: do the same for the shell `index.html` (~163-line `<style>` block). Only after that, reconsider whether a shared `styles/tokens.css` makes sense — Sklad and OSBB currently define genuinely different `:root` token sets (different variable names/scales in places), so a shared file would need a real design decision, not just a mechanical move.
